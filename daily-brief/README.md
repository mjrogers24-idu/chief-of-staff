...
- `src/app/admin/today` — the "Today's Brief" dashboard, the app's home view after sign-in: a mobile-first widget grid (Today's Schedule, Dinner Tonight, Forms & Outstanding — `lucide-react` icons, one column on phones, two on wider screens via `src/components/brief/Widget.tsx`), a weather chip, and an amber alert banner for the prep-ahead/travel notes. Wear/dinner-flag rule matches render as inline badges on their schedule item instead of a separate duplicate list. `dinnerTonight` and `openTasks` are live-overridden from `mealPlans`/`openTasks` on top of the cached `dailyBriefs` snapshot, so same-day edits show up immediately instead of waiting for the next `dailyIngestion` run — the rest of the brief (weather, schedule, prep-ahead, travel) only updates when ingestion re-runs, since it depends on data (calendar fetch, tomorrow's brief) that isn't cheap to duplicate client-side
...
The Family Command Center homepage card (spec 3.5) needs that other app's codebase, which isn't available in this repo/session. Gmail inbox scanning for forms/permission slips (spec 3.1a) was explicitly deferred out of v1 per the section 5 decision. The travel-note keyword ("travel") is hardcoded in `extras.ts` rather than configurable — a single string, not worth a settings page unless it turns out to need one.

The daily brief/email includes *tonight's* planned dinner (pulled from `mealPlans`) but not the full weekly grocery list — that stays on `/admin/meals` only, since it doesn't change day to day and repeating it in every morning's email would just be noise.
