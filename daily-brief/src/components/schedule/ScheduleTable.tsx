...
export function ScheduleTable({ items, onEdit, onDelete }: ScheduleTableProps) {
  if (items.length === 0) {
    return <p className="text-sm text-gray-500">No recurring schedule items yet.</p>;
  }
      ...
      <tbody>
        {items.map((item) => (
          <tr key={item.id} className="border-b border-gray-100">
            ...
            <td className="py-2">
              <button onClick={() => onEdit(item)} className="mr-3 text-gray-700 underline">
                Edit
              </button>
              <button onClick={() => onDelete(item)} className="text-red-600 underline">
                Delete
              </button>
            </td>
          </tr>
