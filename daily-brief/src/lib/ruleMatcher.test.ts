...
describe("matchBriefRules", () => {
  ...
  it("matches case-insensitively", () => {
    const matches = matchBriefRules([item({ title: "pe class", kid: "josh" })], [peJosh]);
    expect(matches).toHaveLength(1);
  });

  it("returns no matches when nothing lines up", () => {
    const matches = matchBriefRules([item({ title: "Library day", kid: "Maddie" })], [peJosh, fieldTrip]);
    expect(matches).toHaveLength(0);
  });

  it("ignores rules with a blank keyword", () => {
    const matches = matchBriefRules(
      [item({ title: "Anything", kid: "Josh" })],
      [{ keyword: "  ", kid: null, wearNote: "x", dinnerFlag: null }],
    );
    expect(matches).toHaveLength(0);
  });
});
